...
class ZoneModelTests(TestCase):
    def setUp(self):
        self.zone_data = {"name": "Test Zone", "is_active": True}
...
class DrawTypeModelTests(TestCase):
    def setUp(self):
        self.draw_type_data = {"code": "TEST", "name": "Test Draw", "is_active": True}
...
class DrawScheduleModelTests(TestCase):
    def setUp(self):
        ...
        self.schedule_data = {
            ...
            "cutoff_time": timezone.localtime()
            .time()
            .replace(hour=18, minute=0, second=0),
            "is_active": True,
        }
    ...
    def test_draw_schedule_str_representation(self):
        schedule = DrawSchedule.objects.create(**self.schedule_data)
        expected_str = (
            f'Test Zone - Test Draw ({schedule.cutoff_time.strftime("%H:%M")})'
        )
...
class ZoneSerializerTests(TestCase):
    def setUp(self):
        self.zone_data = {"name": "Test Zone"}
        self.zone = Zone.objects.create(**self.zone_data)

    def test_zone_serializer_validation(self):
        # Usar datos únicos para evitar conflicto de unicidad
        unique_zone_data = {"name": "Unique Test Zone"}
        serializer = ZoneSerializer(data=unique_zone_data)
...
class NumberLimitSerializerTests(TestCase):
    def setUp(self):
        ...
        self.limit = NumberLimit.objects.create(
            zone=self.zone, draw_type=self.draw_type, number="12", max_pieces=100
        )
...
class ZoneViewSetTests(TestCase):
    ...
    def test_update_zone(self):
        """
        Este test verifica que un usuario administrador pueda actualizar una zona.
        Si el status es 403, probablemente el token no tiene permisos suficientes.
        """
        # Refrescar credenciales por si acaso
        self.client.credentials(
            HTTP_AUTHORIZATION=f"Bearer {RefreshToken.for_user(self.admin_user).access_token}"
        )
        update_data = {"name": "Updated Zone"}
        response = self.client.patch(
            reverse("zone-detail", args=[self.zone.id]), update_data, format="json"
        )
        # Si falla, mostrar el contenido de la respuesta para depuración
        if response.status_code != status.HTTP_200_OK:
            print("Respuesta:", response.status_code, response.data)
        self.assertEqual(response.status_code, status.HTTP_200_OK)
...
class DrawTypeViewSetTests(TestCase):
    ...
    def test_create_draw_type(self):
        new_draw_data = {
            "code": "NEW",
            "name": "New Draw",
            "description": "New draw type",
        }
        response = self.client.post(
            reverse("drawtype-list"), new_draw_data, format="json"
        )
        self.assertEqual(response.status_code, status.HTTP_201_CREATED)
...
class DrawScheduleViewSetTests(TestCase):
    ...
    def test_create_draw_schedule(self):
        ...
        response = self.client.post(
            reverse("drawschedule-list"), new_schedule_data, format="json"
        )
        self.assertEqual(response.status_code, status.HTTP_201_CREATED)
...
class NumberLimitViewSetTests(TestCase):
    ...
    def test_create_number_limit(self):
        ...
        response = self.client.post(
            reverse("numberlimit-list"), new_limit_data, format="json"
        )
        self.assertEqual(response.status_code, status.HTTP_201_CREATED)
...
class CatalogPermissionsTests(TestCase):
    def setUp(self):
        ...
        self.seller_user = User.objects.create_user(
            username="seller", password="seller123", role="VENDEDOR"
        )
